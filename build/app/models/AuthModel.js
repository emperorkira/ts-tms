...
exports.AuthModel = {
    ...
    async generateToken(user) {
        return jsonwebtoken_1.default.sign({ user }, SECRET, { expiresIn: "30m" });
    ...
    async generateRefreshToken(user) {
        return jsonwebtoken_1.default.sign({ user }, REFRESH, { expiresIn: "8h" });
    ...
    async hashPassword(Password) {
        ...
        const cipher = crypto_1.default.createCipheriv(algorithm, Buffer.from(ENCRYPTION, "hex"), iv);
    ...
    async decryptPassword(encryptedPassword) {
        ...
        const decipher = crypto_1.default.createDecipheriv("aes-256-cbc", Buffer.from(ENCRYPTION, "hex"), iv);
    ...
    async createUser(table, recordData) {
        const request = new mssql_1.default.Request();
        if (table === "Ticket") {
            recordData.TicketNumber = await (0, functions_1.generateCodeWithDate)(table);
        }
        else {
            recordData.Code = await (0, functions_1.generateCode)(table);
        }
...
(async () => {
    try {
        const getUsername = await exports.AuthModel.getUserByUsername('User', 'superuser');
